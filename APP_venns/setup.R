# setup workspace prior to initializing ui and server
library("shiny")
library("venneuler")
...
# markData = markData_4me3_4ac colnames(pass75) = paste('perc75', colnames(pass75))
# colnames(pass98) = paste('perc98', colnames(pass98)) percData = cbind(pass75,
# pass98)
pname2pkey = c("perc75", "perc98")
names(pname2pkey) = c("75th Percentile", "98th Percentile")
src_uniq = "Uniquely H3K4"
...
vennCounts = function(x, include = "both") {
    ...
    for (j in 1:ncontrasts) outcomes[, j] <- rep(0:1, times = 2^(j - 1), each = 2^(ncontrasts - 
        j))
    xlist <- list()
    for (i in 1:ncontrasts) xlist[[i]] <- factor(x[, ncontrasts - i + 1], levels = c(0, 
        1))
...
vennDiagram = function(object, include = "both", names = NULL, mar = rep(1, 4), cex = c(1.5, 
    1, 0.7), lwd = 1, circle.col = NULL, counts.col = NULL, show.include = NULL, ...) {
    ...
    par(mar = mar)
    if (nsets <= 3) {
        plot(x = 0, y = 0, type = "n", xlim = c(-4, 4), ylim = c(-4, 4), xlab = "", ylab = "", 
            axes = FALSE, ...)
        ...
        for (circle in 1:nsets) {
            ...
            if (FILL.COL) {
                ...
                polygon(xcentres[circle] + r * cos(theta), ycentres[circle] + r * sin(theta), 
                  border = circle.col[circle], lwd = lwd, col = RGB.ALP)
            }
            text(xtext[circle], ytext[circle], names[circle], cex = cex)
        }
        switch(nsets, rect(-3, -2.5, 3, 2.5), rect(-3, -2.5, 3, 2.5), rect(-3, -3.5, 
            3, 3.3))
        showCounts <- switch(nsets, function(counts, cex, adj, col, leg) {
            ...
            text(0.75, -0.35, counts[4], cex = cex, col = col, adj = adj)
            text(-1.5, 1, counts[5], cex = cex, col = col, adj = adj)
            text(-0.75, -0.35, counts[6], cex = cex, col = col, adj = adj)
        ...
        if (LenInc == 1) 
            adj <- c(0.5, 0.5) else adj <- c(0.5, 0)
        showCounts(counts = z, cex = cex[1], adj = adj, col = counts.col[1], leg = include[1])
        if (LenInc == 2) 
            showCounts(counts = z2, cex = cex[1], adj = c(0.5, 1), col = counts.col[2], 
                leg = include[2])
        return(invisible())
    }
    ...
    if (4 == nsets) {
        rect(-20, -20, 420, 400)
        elps <- cbind(162 * cos(seq(0, 2 * pi, len = 1000)), 108 * sin(seq(0, 2 * pi, 
            len = 1000)))
        if (!FILL.COL) {
