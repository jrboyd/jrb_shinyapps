...
shinyServer(function(input, output, session) {
  ...
  output$downloadData <- downloadHandler(contentType = 'application/pdf',
    filename = function() { paste(tempdir(),'heatmap.pdf', sep='') },
    content = function(file) {
      fname = file
      
      if (file.exists(paste0(fname, ".pdf")))
        file.rename(paste0(fname, ".pdf"), fname)  
      print(fname)
      pdf(fname)
      ...
      options(warn = -1)
      #sink('NUL')
      res_tmp = heatmap.3(get_markData()[getKeep(), ], trace = "n", classCount = getNumClasses(), main = getTitle(), margins = c(.05,2), cexCol = 3, key = F, colsep = 3)
      tmp = res_tmp[[3]]
  ...
  output$davidURL = renderUI({
    ...
    address = paste('http://david.abcc.ncifcrf.gov/api.jsp?',
                    ...
                    '&tool=',tool,
                    '&annot=',annot, sep = '')
    textInput('davidURL', label = 'DAVID Query', value = address)
  })
