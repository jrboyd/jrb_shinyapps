...
shinyServer(function(input, output, session) {
  fetchClass=function(n, res){
    ...
    nth_class = asPlotted[start:end,,drop = F]
    return(nth_class)
  }
  ...
  getKeep = reactive({
    md = get_markData()
    trans = colnames(md)#if ac we want me, and vice versa
    names(trans) = colnames(md)[c(4:6,1:3)]
    p1 = input$p1
    p2 = input$p2
    threshold = log2(getThreshold())
    #up and trans down
    upDown = ( (md[,p1] - md[,p2]) > threshold) & ( (md[,trans[p1]] - md[,trans[p2]]) < -threshold)
    downUp = ( (md[,p1] - md[,p2]) < -threshold) & ( (md[,trans[p1]] - md[,trans[p2]]) > threshold)
    #or down and trans up
    keep = upDown | downUp
    return(keep)
  })
